...
- **`internal/auth.LocalAdminEmail()` is an exported accessor function, NOT an exported constant.** Returns the unexported `localAdminEmail` ("admin@localhost"). Multi-user paths MUST use the actual submitted username, NOT this accessor. The `internal/api/rest` package's `localAuthUsername` constant is the OSS single-user rate-limit key; `TestLocalAuthUsernameMatchesAuthPackageAccessor` enforces their equality. Do NOT export the constant itself — that invites the cross-package import that CLAUDE.md's existing guardrail forbids.
