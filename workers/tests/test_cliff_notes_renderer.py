...
async def test_deep_repository_entity_sections_get_deterministic_leads() -> None:
    provider = _RecordingProvider(
        response_text=json.dumps(
            [
                {
                    "title": title,
                    "content": "This section starts generically. While diagrams are mentioned, direct evidence is thin. It later adds grounded details.",
                    "summary": "Generic summary.",
                    "confidence": "high",
                    "inferred": False,
                    "evidence": [
    ...
    assert "While diagrams are mentioned" not in by_title["Domain Model"].content
