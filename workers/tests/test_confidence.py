...
def test_single_signal_no_boost():
    ...
    links = [
        Link(requirement_id="REQ-001", entity=_make_entity(), source=LinkSource.COMMENT,
             link_type=LinkType.IMPLEMENTS, confidence=0.95),
    ]
...
def test_multi_signal_boost():
    ...
    entity = _make_entity()
    links = [
        Link(requirement_id="REQ-001", entity=entity, source=LinkSource.COMMENT,
             link_type=LinkType.IMPLEMENTS, confidence=0.95),
        Link(requirement_id="REQ-001", entity=entity, source=LinkSource.SEMANTIC,
             link_type=LinkType.IMPLEMENTS, confidence=0.75),
    ]
    scored = score_links(links)
    assert len(scored) == 1
...
def test_confidence_between_zero_and_one():
    ...
    links = [
        Link(requirement_id="REQ-001", entity=entity, source=LinkSource.COMMENT,
             link_type=LinkType.IMPLEMENTS, confidence=0.99),
        Link(requirement_id="REQ-001", entity=entity, source=LinkSource.SEMANTIC,
             link_type=LinkType.IMPLEMENTS, confidence=0.99),
        Link(requirement_id="REQ-001", entity=entity, source=LinkSource.TEST,
             link_type=LinkType.TESTS, confidence=0.99),
    ]
    scored = score_links(links)
...
def test_different_entities_not_merged():
    ...
    links = [
        Link(requirement_id="REQ-001", entity=_make_entity("funcA"),
             source=LinkSource.COMMENT, link_type=LinkType.IMPLEMENTS, confidence=0.9),
        Link(requirement_id="REQ-001", entity=_make_entity("funcB"),
             source=LinkSource.COMMENT, link_type=LinkType.IMPLEMENTS, confidence=0.8),
    ]
    scored = score_links(links)
    assert len(scored) == 2
...
def test_different_reqs_not_merged():
    ...
    entity = _make_entity()
    links = [
        Link(requirement_id="REQ-001", entity=entity,
             source=LinkSource.COMMENT, link_type=LinkType.IMPLEMENTS, confidence=0.9),
        Link(requirement_id="REQ-002", entity=entity,
             source=LinkSource.COMMENT, link_type=LinkType.IMPLEMENTS, confidence=0.8),
    ]
    scored = score_links(links)
    assert len(scored) == 2
...
def test_comment_test_boost():
    ...
    links_ct = [
        Link(requirement_id="REQ-001", entity=entity, source=LinkSource.COMMENT,
             link_type=LinkType.IMPLEMENTS, confidence=0.90),
        Link(requirement_id="REQ-001", entity=entity, source=LinkSource.TEST,
             link_type=LinkType.TESTS, confidence=0.85),
    ]
    links_cs = [
        Link(requirement_id="REQ-002", entity=_make_entity("funcB"), source=LinkSource.COMMENT,
             link_type=LinkType.IMPLEMENTS, confidence=0.90),
        Link(requirement_id="REQ-002", entity=_make_entity("funcB"), source=LinkSource.SEMANTIC,
             link_type=LinkType.IMPLEMENTS, confidence=0.75),
    ]
