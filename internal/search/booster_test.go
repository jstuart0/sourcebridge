...
func TestRequirementBooster_NoOpOnZeroLinkRepo(t *testing.T) {
	...
	b.Apply(t.Context(), results, &Request{Repo: repoID, Query: "x"}, RouterOutput{})
...
func TestRequirementBooster_LiftsLinkedSymbols(t *testing.T) {
	...
	syms, _ := s.GetSymbols(t.Context(), repoID, nil, nil, 1, 0)
	if len(syms) == 0 {
	...
	s.StoreLink(t.Context(), repoID, linkIn)
...
func TestRequirementBooster_InvalidateRehydrates(t *testing.T) {
	...
	b.Apply(t.Context(), warm, &Request{Repo: repoID}, RouterOutput{})
	...
	b.Apply(t.Context(), stale, &Request{Repo: repoID}, RouterOutput{})
	...
	b.Apply(t.Context(), fresh, &Request{Repo: repoID}, RouterOutput{})
