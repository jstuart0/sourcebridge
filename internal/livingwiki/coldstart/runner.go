...
//
// GQL-5 (bob A-H4): moved from internal/api/graphql/living_wiki_coldstart.go.
//
// BuildRunner and Config are the public surface added by this move. All other
// symbols in this file remain package-private (unexported).
//
// This file provides:
//   - [BuildRunner] — the RunWithContext closure injected into
...
const (
	...
	// defaultTenantID is the single-tenant sentinel used for all repo-settings
	// and credential lookups. Inlined here from the graphql package constant so
	// this package has no dependency on the graphql layer.
...
// Config consolidates the parameters for BuildRunner.
...
//
// GQL-5: exported from this package (coldstart) after being moved from
// internal/api/graphql. Previously named coldStartConfig (unexported).
type Config struct {
...
//
// GQL-5: exported from this package (coldstart) after being moved from
// internal/api/graphql. Previously named buildColdStartRunner (unexported).
func BuildRunner(cfg Config) func(ctx context.Context, rt llm.Runtime) error {
	if cfg.Mode == "" {
	...
	return func(runCtx context.Context, rt llm.Runtime) error {
		...
		for _, p := range pages {
			// Classify using lworch.PageKind* constants directly (no GQL type deps).
			// Prefer Kind when set; fall back to TemplateID for legacy plans.
			switch {
			case p.Kind == lworch.PageKindRepoWide ||
				(p.Kind == lworch.PageKindUnknown && repoWideTemplateIDs[p.TemplateID]):
				repoWidePageCount++
			case p.Kind == lworch.PageKindCluster ||
				(p.Kind == lworch.PageKindUnknown && p.TemplateID == templateIDArchitecture):
				clusterPageCount++
			default:
		...
		// BuildRunner closures for the same repo.
		//
		...
		case errors.Is(err, lworch.ErrSystemicSoftFailures):
			...
			status = "partial"
			failCat = FailureCategorySystemicLLM
			errMsg = err.Error()
		...
		case err != nil && isPartial:
			...
			status = "partial"
			failCat = FailureCategoryPartialContent
			errMsg = err.Error()
...
// DispatchGeneratedPages is the exported form of dispatchGeneratedPages.
// Used by the on-demand generation path in schema.resolvers.go which needs to
// dispatch a single freshly-generated page to sinks without a full cold-start.
func DispatchGeneratedPages(
	...
	return dispatchGeneratedPages(ctx, repoID, tenantID, generatedPages, skippedPageIDs, broker, repoSettingsStore, repoName, status, failCat, errMsg, mode)
}
...
// ColdStartTimeBudget is the maximum wall-clock time for a single cold-start
// (or retry-excluded) generation run. Exported so the on-demand generation
// path in schema.resolvers.go can use the same budget constant.
const ColdStartTimeBudget = coldStartTimeBudget
...
// ResolveTaxonomyForMode is the exported form of resolveTaxonomyForMode.
// Used by graphql resolvers that build their own taxonomy (plan-preview,
// EnableLivingWikiForRepo signature validation).
func ResolveTaxonomyForMode(ctx context.Context, mode, repoID string, gs graphstore.GraphStore, lc *llmcall.Caller, cs clustering.ClusterStore) ([]lworch.PlannedPage, error) {
	return resolveTaxonomyForMode(ctx, mode, repoID, gs, lc, cs)
}

// ResolveTaxonomy is the exported backward-compat wrapper (lw_detailed mode).
// Used by graphql-package tests.
func ResolveTaxonomy(ctx context.Context, repoID string, gs graphstore.GraphStore, lc *llmcall.Caller, cs clustering.ClusterStore) ([]lworch.PlannedPage, error) {
	return resolveTaxonomy(ctx, repoID, gs, lc, cs)
}

// AttachKnowledgeArtifacts is the exported form of attachKnowledgeArtifacts.
// Used by graphql-package tests.
func AttachKnowledgeArtifacts(ctx context.Context, repoID string, pages []lworch.PlannedPage, ks knowledge.KnowledgeStore) []lworch.PlannedPage {
	return attachKnowledgeArtifacts(ctx, repoID, pages, ks)
}

// BuildExclusionReasons is the exported form of buildExclusionReasons.
// Used by graphql-package tests.
func BuildExclusionReasons(excluded []lworch.ExcludedPage) []string {
	return buildExclusionReasons(excluded)
}

// ClassifyPage is the exported form of classifyPage.
// Used by graphql-package tests.
func ClassifyPage(
	pageID string,
	...
	return classifyPage(pageID, alreadyPublished, currentFp, persistedFps, writers)
}
...
// NewGraphStoreSymbolGraph creates a templates.SymbolGraph adapter backed by
// a graph.GraphStore. Used by on-demand generation in schema.resolvers.go.
func NewGraphStoreSymbolGraph(store graphstore.GraphStore) templates.SymbolGraph {
...
// NewLLMCaller creates a templates.LLMCaller adapter backed by an
// llmcall.Caller. Used by on-demand generation in schema.resolvers.go when
// it needs to drive a single-page LLM call using the same adapter contract.
func NewLLMCaller(caller *llmcall.Caller, repoID, op string) templates.LLMCaller {
	return &coldStartLLMCaller{caller: caller, repoID: repoID, op: op}
}
...
// BucketRegenerate, BucketSkipFully, BucketSkipNeedsFixup are exported forms
// of the 3-way smart-resume bucket constants, used by graphql-package tests.
const (
	BucketRegenerate     = bucketRegenerate
	BucketSkipFully      = bucketSkipFully
	BucketSkipNeedsFixup = bucketSkipNeedsFixup
)
...
// (BuildRunner Step 1.65) and GenerateLivingWikiPageOnDemand in
// schema.resolvers.go. Both callers now access this via the coldstart package.
//
...
// NewRegistryTierFunc is the exported accessor for newRegistryTierFunc, used
// by callers in other packages (e.g. schema.resolvers.go) that need the same
// registry-backed tier resolution as the cold-start runner.
func NewRegistryTierFunc(store comprehension.Store) modeltier.TierFunc {
	return newRegistryTierFunc(store)
}
...
// ─────────────────────────────────────────────────────────────────────────────
// Page-count cap and selection helpers
// ─────────────────────────────────────────────────────────────────────────────
...
//
// GQL-5: exported so the graphql package can delegate to this package for the
// shared cap logic used by both coldstart and plan-preview.
func ApplyPageCap(
	pages []lworch.PlannedPage,
	maxPagesPerJob int,
...
//
// GQL-5: exported so the graphql package can delegate here for the shared
// selection logic used by both coldstart and plan-preview.
...
//
// GQL-5: exported so the graphql package shim can delegate here.
...
// ─────────────────────────────────────────────────────────────────────────────
// Index-page write serialization
// ─────────────────────────────────────────────────────────────────────────────
...
// LD-12) build independent runner closures via BuildRunner, so a
...
// For 10 K repos that is ~240 KB — acceptable.
...
// Append adds s to the accumulator in a concurrency-safe manner.
...
// append is an unexported alias for Append, used within this package.
func (a *AtomicStringSlice) append(s string) { a.Append(s) }

// Snapshot returns a point-in-time copy of the accumulated strings.
