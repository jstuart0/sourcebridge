...
describe("SourceBridgeClient", () => {
  ...
  it("throws on GraphQL errors", async () => {
    ...
    await expect(client.query("query { bad }")).rejects.toThrow("GraphQL errors");
  });
  ...
  it("throws on HTTP errors", async () => {
    ...
    await expect(client.query("query { bad }")).rejects.toThrow("GraphQL request failed: 500");
  });
...
describe("SourceBridgeClient.isServerRunning with /readyz", () => {
  let client: SourceBridgeClient;
  ...
  it("calls the /readyz endpoint, not /api/v1/health", async () => {
    mockFetch.mockResolvedValueOnce({ ok: true });
